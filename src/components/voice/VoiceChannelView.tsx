...
export function VoiceChannelView({ channelId, channelName, participants, onStartScreenShare }: VoiceChannelViewProps) {
    ...
    useEffect(() => {
        screenSharingParticipants.forEach(participant => {
            const video = videoRefs.current.get(participant.user_id);
                ...
                {screenSharingParticipants.length === 0 ? (
                    ...
                    <div className="h-full flex flex-col items-center justify-center">
                        ...
                        <h3 className="text-xl font-semibold text-white mb-2">
                            Kimse ekran paylaşmıyor
                        </h3>
                        <p className="text-gray-400 mb-4 text-center max-w-md">
                            Ekranınızı paylaşmak için aşağıdaki "Ekran Paylaş" butonuna tıklayın
                        </p>
                        <button
                    ...
                    // Vertical list of screen shares
                        ...
                        {screenSharingParticipants.map((participant) => (
                            ...
                            >
                                <video
                                    ref={(el) => {
                                        if (el) {
                                            videoRefs.current.set(participant.user_id, el);
                                        } else {
                                            videoRefs.current.delete(participant.user_id);
                                        }
                                    }}
                                    autoPlay
                                    playsInline
