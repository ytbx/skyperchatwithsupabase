        ...
        {loading ? (
          ...
          <>
            {groupMessages(messages).map((group, groupIndex) => {
              ...
              const senderName = isMe ? (profile?.username || 'Ben') : contactName;
              const senderImage = isMe ? profile?.profile_image_url : contactProfileImageUrl;
              ...
              return (
                <div key={groupIndex} className="message-group hover:bg-gray-900/20 transition-colors duration-150 rounded px-2 py-0.5 -mx-2">
                  <div className="flex gap-3 pt-1">
                    <div className="w-10 h-10 rounded-full bg-primary-500 flex-shrink-0 flex items-center justify-center overflow-hidden transition-transform duration-200 hover:scale-105 cursor-pointer">
                      {senderImage ? (
                        <img
                          src={senderImage}
                          alt={senderName}
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <span className="text-xs text-gray-500 font-medium">
                          {formatMessageTime(group[0].created_at)}
                        </span>
                      </div>
